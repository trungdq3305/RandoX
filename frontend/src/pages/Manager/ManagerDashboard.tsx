...
const ManagerDashboard: React.FC = () => {
  ...
  const stats = [
    { title: 'Tổng sản phẩm', value: products.length, icon: '📦', color: '#f093fb' },
    { title: 'Sản phẩm có sẵn', value: products.filter(p => p.status === 'available').length, icon: '✅', color: '#4caf50' },
    { title: 'Hết hàng', value: products.filter(p => p.status === 'out_of_stock').length, icon: '⚠️', color: '#ff9800' },
    { title: 'Tổng giá trị kho', value: formatPrice(products.reduce((sum, p) => sum + (p.price * p.stock), 0)), icon: '💎', color: '#e91e63' },
  ]
